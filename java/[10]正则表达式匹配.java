...
class Solution10 {
        ...
        for (int i = 0; i < s.length(); i++) {
            for (int j = 0; j < p.length(); j++) {
                if (p.charAt(j) >= 'a' && p.charAt(j) <= 'z') {
                    state[i + 1][j + 1] = state[i][j] && s.charAt(i) == p.charAt(j);
                } else if (p.charAt(j) == '.') {
                    ...
                    if (p.charAt(j - 1) != s.charAt(i) && p.charAt(j - 1) != '.') {
                        ...
                        state[i + 1][j + 1] = state[i][j + 1] || state[i + 1][j] || state[i + 1][j - 1];
                    }
                }
            }
        }
    ...
    public static void main(String[] args) {
        ...
        System.out.println(solution10.isMatch("aa", ".*"));
        System.out.println(solution10.isMatch("aa", "a*"));
        System.out.println(solution10.isMatch("aa", "a"));
        System.out.println(solution10.isMatch("aaasdg", ".*"));
    }
}
